  ...
  return (
   ...
   >
    ...
    <aside
      ...
      className={cn(
        "absolute right-0 top-0 flex h-full w-[min(88vw,420px)] flex-col border-l border-[color:rgb(200_169_106_/_0.25)] bg-[var(--color-black-900)] p-6 text-[var(--color-ivory-100)] shadow-[var(--shadow-modal)] transition-transform duration-[var(--duration-slow)]",
     ...
     <nav className="mt-12 grid gap-2" aria-label="Mobile navigation">
      {links.map((link) => (
        ...
        <Link
          ...
          href={link.href}
          onClick={onClose}
          className="rounded-md px-4 py-3 text-lg transition-colors hover:bg-white/5 hover:text-[var(--color-gold-500)]"
        >
          {link.label}
        </Link>
      ))}
